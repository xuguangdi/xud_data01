...
def plot_training_curves(history_dict, zs_mean, zs_std, save_path):
    ...
    plt.title('Performance Comparison: Strawberry Diseases')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy (%)')
    plt.legend()
...
class StrawberryDataset(Dataset):
    def __init__(self, root_dir, split, transform=None):
        ...
        for class_name in self.classes:
            cls_dir = os.path.join(target_dir, class_name)
    ...
    def init_prototypes(self, tokenizer, class_names, device):
        ...
        with torch.no_grad():
            for cls in class_names:
                ...
                toks = tokenizer([f"a photo of {clean_cls}."]).to(device)
...
class CoOp(nn.Module):
    ...
    def init_prototypes(self, tokenizer, class_names, device):
        ...
        prompts = [f"{dummy_ctx} {c.replace('_', ' ')}." for c in class_names]
...
class TipAdapterF(nn.Module):
    ...
    def init_prototypes(self, tokenizer, class_names, device):
        ...
        with torch.no_grad():
            for cls in class_names:
                clean_cls = cls.replace("_", " ")
                prompts = [f"a photo of {clean_cls}."]
...
class DPAFNet(nn.Module):
    ...
    def load_knowledge(self, tokenizer, class_names, json_path, device):
        ...
        with torch.no_grad():
            for cls in class_names:
                clean_cls = cls.replace("_", " ")
                prompts = [
                    f"a photo of {clean_cls}.",
                    f"a close-up photo of {clean_cls}, a strawberry disease."
                ]
                ...
                elif descs and isinstance(descs, str):
    ...
    def forward(self, images):
        ...
        logits = w * logits_txt + (1 - w) * logits_vis
...
def run_model(model_name, device, tokenizer, cycle_idx):
    ...
    train_ds = StrawberryDataset(DATASET_ROOT, 'train', transform=train_aug)
        ...
        for ep in range(epochs):
            ...
            if acc > best_acc:
                ...
                if cycle_idx == NUM_CYCLES and model_name == "DPAFNet":
                    cm_path = os.path.join(RESULTS_DIR, f"cm_DPAFNet_Best.png")
...
def main():
    ...
    print(f"\n{'=' * 50}\nFINAL COMPARISON RESULTS ({NUM_CYCLES} Runs)\n{'=' * 50}")
    zs_mean = 0.0
    ...
    plot_training_curves(curves, zs_mean, 0.0, os.path.join(RESULTS_DIR, "comparison_curves.png"))
